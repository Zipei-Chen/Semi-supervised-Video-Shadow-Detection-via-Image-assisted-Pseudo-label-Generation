...
import joint_transforms2 as joint_transforms2
import joint_transforms
from config import ViSha_training_root
from config import ViSha_validation_root
from config import ViSha_pseudo_training_root
from dataset.VShadow_crosspairwise_removal import CrossPairwiseImg, VSD_Dataset, pseudo_VSD_Dataset
...
from networks.MPLNet import MPLNet
...
writer = SummaryWriter(log_dir='./mplnet1')
...
joint_transform2 = joint_transforms2.Compose([
    joint_transforms2.RandomHorizontallyFlip(),
    joint_transforms2.Resize((args['scale'], args['scale']))
])
...
pseudo_training_root = [ViSha_pseudo_training_root]  # training_root should be a list form, like [datasetA, datasetB, datasetC], here we use only one dataset.
pseudo_train_set = pseudo_VSD_Dataset(pseudo_training_root, joint_transform2, img_transform, target_transform, )
pseudo_train_loader = DataLoader(pseudo_train_set, batch_size=batch_size, num_workers=8, shuffle=True)

print('=====>Validation Dataset loading<======')
validation_root = [ViSha_validation_root]
validation_set = VSD_Dataset(validation_root, val_joint_transform, img_transform, target_transform=target_transform,
                             is_train=False)
...
def main():
    ...
    # multi-GPUs training
    if args['multi-GPUs']:
        net = torch.nn.DataParallel(MPLNet()).cuda().train()
...
def train(net, optimizer, gma):
    ...
    while True:
        ...
        for i, sample in enumerate(
                tqdm(train_loader, desc=f'Epoch: {curr_epoch}', ncols=100, ascii=' =', bar_format='{l_bar}{bar}|')):
            ...
            exemplar_resize = sample['exemplar_resize'].cuda()
            for i in range(len(exemplar_resize)):
                exemplar_resize[i] = exemplar_resize[i] * 255.0
            ...
            flow4 = [torch.zeros([batch_size, 2, 25, 25])]
            for i in range(len(exemplar_resize)-1):
                _, flow_example = gma(exemplar_resize[i+1], exemplar_resize[i], iters=6, test_mode=True)
                flow_example = resize_flow(flow_example, [25, 25])
                flow4.append(flow_example)
            ...
            with autocast():
                ...
                pre_prediction = net(exemplar, flow4)
                ...
                for i in range(len(exemplar)):
                    bce_loss.append((binary_xloss(pre_prediction[i][0], exemplar_gt[i]) + binary_xloss(
                        pre_prediction[i][1], exemplar_gt[i])
                                     + binary_xloss(pre_prediction[i][2], exemplar_gt[i]) + binary_xloss(
                                pre_prediction[i][3], exemplar_gt[i])
                                     + binary_xloss(pre_prediction[i][4], exemplar_gt[i]) + binary_xloss(
                                pre_prediction[i][5], exemplar_gt[i])
                                     ) / 6.0)
                # bce_loss1 = (binary_xloss(pre_prediction[0], exemplar_gt) + binary_xloss(pre_prediction[1], exemplar_gt)
                #              + binary_xloss(pre_prediction[2], exemplar_gt)) / 3.0
                ...
                bce_loss1 = bce_loss[0]
                for i in range(len(exemplar) - 1):
                    bce_loss1 += bce_loss[i + 1]
                loss_seg = bce_loss1
            ...
            log = "iter: %d, bce1: %f5, lr: %f8" % \
                  (curr_iter, loss_record1.avg, optimizer.param_groups[0]['lr'])
        ...
        for i, sample in enumerate(
            tqdm(pseudo_train_loader, desc=f'Epoch: {curr_epoch}', ncols=100, ascii=' =', bar_format='{l_bar}{bar}|')):

            exemplar, exemplar_gt, uncertain = sample['exemplar'].cuda(), sample['exemplar_gt'].cuda(), sample[
                'uncertainty']
            # ref2 = sample['ref2'].cuda()
            exemplar_resize = sample['exemplar_resize'].cuda()
            for i in range(len(exemplar_resize)):
            ...
            with autocast():
                ...
                bce_loss = []
                for i in range(len(exemplar)):
                    binary_xloss = nn.BCEWithLogitsLoss(weight=uncertain[i]).cuda()
                    bce_loss.append((binary_xloss(pre_prediction[i][0], exemplar_gt[i]) + binary_xloss(pre_prediction[i][1],exemplar_gt[i])
                                 + binary_xloss(pre_prediction[i][2], exemplar_gt[i]) + binary_xloss(pre_prediction[i][3],exemplar_gt[i])
                                 + binary_xloss(pre_prediction[i][4], exemplar_gt[i]) + binary_xloss(pre_prediction[i][5],exemplar_gt[i])
                                 )/6.0)
                # bce_loss1 = (binary_xloss(pre_prediction[0], exemplar_gt) + binary_xloss(pre_prediction[1], exemplar_gt)
                #              + binary_xloss(pre_prediction[2], exemplar_gt)) / 3.0
            ...
            loss_record2.update(bce_loss1.item(), batch_size)
            # loss_record2.update(bce_loss2.item(), batch_size)
            # loss_record3.update(loss_hinge1.item(), batch_size)

            curr_iter += 1

            log = "iter: %d, bce2: %f5, lr: %f8" % \
                  (curr_iter, loss_record2.avg, optimizer.param_groups[0]['lr'])
        ...
        writer.add_scalar('training_pseudo_bce_loss', loss_record2.avg, curr_epoch)
        ...
        #             # images = []
        #             exemplar, exemplar_gt = data['exemplar'].cuda(), data['exemplar_gt'].cuda()
        #             # ref2 = sample['ref2'].cuda()
        #             exemplar_resize = data['exemplar_resize'].cuda()
        #             for i in range(len(exemplar_resize)):
        #                 exemplar_resize[i] = exemplar_resize[i] * 255.0
        #
