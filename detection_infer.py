    ...
    with torch.no_grad():
        ...
        for i in range(len(path1)):
            ...
            for idx, image_name in enumerate(inputs):
                exemplar = Image.open(image_name).convert('RGB')
                w, h = exemplar.size
                exemplar = transforms.Resize((args['scale'], args['scale']))(exemplar)
                if idx-1 > 0:
                    exemplar_resize = transforms.Resize([384, 384])(exemplar)
                    last_frame = Image.open(inputs[idx-1]).convert('RGB')
                    last_frame_resize = transforms.Resize([384, 384])(last_frame)
                    exemplar_resize = transforms.ToTensor()(exemplar_resize).unsqueeze(0).cuda() * 255.0
                    last_frame_resize = transforms.ToTensor()(last_frame_resize).unsqueeze(0).cuda() * 255.0
                    _, flow = gma(exemplar_resize, last_frame_resize, iters=6, test_mode=True)
                    flow = resize_flow(flow, [25, 25])
                else:
                    flow = torch.zeros([1, 2, 25, 25])

                exemplar = img_transform(exemplar).unsqueeze(0).cuda()
                last_frame_feature, memory, preds = net.recurrent_forward(exemplar, last_frame_feature, memory, flow4=flow)
